...
class CodeGrammar extends EditGrammarAdapter {
    ...
    public List<EditToken> format(@NonNull Editable editable) {
        ...
        List<Pair<Integer, Integer>> list = com.yydcdut.rxmarkdown.syntax.text.CodeGrammar.find(editable.toString());
        for (int i = list.size() - 1; i >= 0; i--) {
            Pair<Integer, Integer> pair = list.get(i);
            int start = pair.first;
            int end = pair.second;
            List<Integer> middleList = com.yydcdut.rxmarkdown.syntax.text.CodeGrammar.getMiddleNewLineCharPosition((SpannableStringBuilder) editable, start, end);
            int current = start;
            ...
            editTokenList.add(new EditToken(mdCodeSpan, end,
                    end + com.yydcdut.rxmarkdown.syntax.text.CodeGrammar.KEY_CODE.length() + (end + com.yydcdut.rxmarkdown.syntax.text.CodeGrammar.KEY_CODE.length() >= editable.length() ? 0 : 1),
